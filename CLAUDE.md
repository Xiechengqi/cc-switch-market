...
- **Clean Borders**: Elements rely on crisp 2px borders, bold color blocking, and layered shapes to create a tactile feel without offset drop shadows.
...
```
No drop shadow.
Use 2px borders, strong color contrast, layered background shapes,
and subtle scale/translate/rotation for tactile feedback.
```

Depth should come from composition, spacing, contrast, and decorative geometry — not from offset shadows.

Recommended interaction feedback:
- hover: slight scale (`scale(1.01~1.02)`) or tiny rotate (`-0.5deg ~ 0.5deg`)
- active: reset scale, keep border contrast
- focus: high-contrast ring or border color change
- never add blurred shadow; avoid offset hard-shadow as well

Use decorative circles, stripes, dots, badges, or nested surfaces to create hierarchy.

Avoid:
- `box-shadow: 4px 4px 0 ...`
- `box-shadow-hover: 6px 6px 0 ...`
- `box-shadow-active: 2px 2px 0 ...`
- any pattern that simulates thickness through border shadows

Instead prefer:
- double-surface layouts
- colored outline accents
- inset color panels
- playful floating shapes behind cards
- spacing and radius contrast

No blur. No offset shadow.

Recommended examples:
- card: 2px border + white surface + decorative accent chip or background shape
- button: 2px border + pill radius + color fill + tiny scale on hover
- input: 2px border + accent border on focus + optional ring, but no shadow

Potential implementation cues:
- cards can use a secondary decorative layer (`before`/absolute shape) behind the main surface
- call-to-action buttons can use icon motion instead of shadow growth
- featured panels can use thicker border, contrast fill, or rotated badge instead of a shadow

Hierarchy rule:
- primary emphasis = color + scale + icon
- secondary emphasis = border + label + spacing
- decorative emphasis = shapes/patterns only

This project should not use border-offset shadow styling going forward.

No shadow. Still playful.

The system should feel bold through geometry, not elevation.

The design language now favors: **flat tactile surfaces, crisp outlines, and lively decoration**.

The goal is to keep the Memphis/playful energy while removing the faux paper-cut shadow treatment.

When implementing UI, delete existing shadow utilities if they are only used to create offset border shadows.

If a component feels too flat after removing shadows, solve it with:
- stronger palette contrast
- more deliberate icon containers
- decorative edge accents
- denser spacing rhythm
- local background patterns
rather than reintroducing shadow.

This shadowless rule applies to:
- buttons
- cards
- inputs
- nav pills
...
Only exception: browser-native focus glow may be replaced by explicit ring styles, but not by box-shadow elevation.


Potential utility direction:
- `.outline-accent`
- `.surface-flat`
- `.interactive-scale`
- `.decorative-chip`
- `.pattern-panel`
rather than `.pop-shadow`.


The visual personality should remain:
**Friendly. Tactile. Pop. Energetic.**
But now expressed with **flat bold surfaces** instead of shadow-stacked stickers.


Do not preserve old shadow classes for backward compatibility unless required during a short migration step.
Prefer deleting them once components are migrated.


Design review test:
If you remove all shadows from a screen and it loses structure, the structure was too dependent on shadows.
Rebuild hierarchy using layout and color instead.


A successful implementation should still feel fun even when every `box-shadow` is removed.


This is a deliberate aesthetic shift, not a temporary simplification.


Use motion sparingly, shapes confidently, and borders clearly.


The final look should feel closer to:
- printed sticker sheets
- color-blocked editorial UI
- playful posters
than to elevated paper cards.


In short:
**Replace elevation with composition.**
